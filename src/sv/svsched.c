/*
    \ \____/ /  \ \
   _/ / __ \ \__/ /     svsched.c - supervisor scheduler
   __/ /  \ \____/
...
/******************************************************************************/
{
  if (runnable_list_count) {
    ...
    --runnable_list_count;
    if (runnable_list_count) {
      memmove(runnable_list,
              runnable_list + 1,
              runnable_list_count * sizeof(task_t *));
    }
    return task;
  }
  return NULL;
...
/******************************************************************************/
{
  for (uint32_t i = 1; i < TASK_COUNT; ++i) {
    if (tasks[i].state == WAITING && tasks[i].waiting_for.event == obj.event) {
      if (obj.event == TASK_COMPLETION &&
          obj.task_id == tasks[i].waiting_for.task_id) {
        tasks[i].waiting_for = NOT_WAITING;
...
/******************************************************************************/
{
  ...
  if (task->state != INACTIVE) {
    cc_printf("   wakeup_in=%x  running=%x",
              task->ticks_until_wakeup,
              task->ticks_running);
  }
  cc_printf("\n");
}
...
/******************************************************************************/
{
  ...
  int iter = 8192;
  printf(
    "TEST: Runnables list implementation randomized test with %i iterations\n",
    iter);
...
/******************************************************************************/
{
  ...
  if (running_count != 1) {
    fprintf(
      stderr,
      "Assertion failed at line %d: There is more than one process running!\n",
      line);
    sv_print_tasks();
