/*
    \ \____/ /  \ \
   _/ / __ \ \__/ /     ccio.c - console controller i/o functions
   __/ /  \ \____/
...
#define _va_sizeof(t)                                                          \
  (((sizeof(t) + sizeof(unsigned) - 1) / sizeof(unsigned)) * sizeof(unsigned))
...
{
  _curr_x =
    (x > ia32_video_get_line_len() - 1) ? ia32_video_get_line_len() - 1 : x;
  _curr_y =
...
/**************************************************************************************************/
{
  ...
  if (keybd_last_key != CC_KEY_LF) {
    keybd_buf_len =
      (keybd_buf_len == KEYBD_BUF_LEN - 1) ? KEYBD_BUF_LEN - 2 : keybd_buf_len;
